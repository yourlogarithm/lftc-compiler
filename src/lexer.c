...
void consume_or_raise(char** pch, char expected, unsigned int code, TokenArray* arr,  int* position, unsigned int line) {
    if (*(pch+1) != expected) {
        err("expected `%c` but found `%c`", expected, *pch);
    }
    (*pch) += 2;
    (*position)++;
    addToken(&arr, position, line, code, NULL);
}
    ...
    while (1) {
        switch (*pch) {
            ...
            case '"':
                ...
                break;
            case '&':
                consume_or_raise(&pch, '&', AND, &arr, &position, line);
                break;
            case '|':
                consume_or_raise(&pch, '|', OR, &arr, &position, line);
